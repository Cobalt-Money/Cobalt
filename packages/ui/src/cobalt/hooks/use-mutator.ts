...
export interface MutateOpts {
  /** Toast text shown when no specific server error message is returned. */
  fallback?: string;
...
/**
 * Shared Zero mutator runner. Fires `build(mutators)` and surfaces server
 * errors as a toast unless overridden. Pulled out of `apps/web/src/hooks` so
 * both the web route and the friends-app txn panel use one implementation.
 */
