...
export interface TagOption {
  ...
  name: string;
  color: TagColor;
  /** Total transactions tagged with this tag (independent of current filters). */
...
interface TagPickerListProps {
  ...
  /** Host hook fired right after `onRequestCreate` (e.g. close the popover). */
  onAfterCreate?: () => void;
...
/**
 * Search + multi-select tag list. Host-agnostic body shared by the standalone
 * {@link TagPicker} popover and the transactions table's context menu, so the
 * picker looks identical wherever it appears.
 */
...
export function TagPicker({
  ...
  return (
    <Popover onOpenChange={setOpen} open={open}>
      ...
      />
      <PopoverContent align={align} className="w-auto gap-0 bg-popover p-1 dark:bg-popover">
        <TagPickerList
