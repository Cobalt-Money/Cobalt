...
interface UseChartInteractionParams {
  ...
  canInteract: boolean;
  /** Fires synchronously when tooltip target changes (event-driven, no effect). */
...
export function useChartInteraction({
  ...
  const [tooltipData, setRawTooltipData] = useState<TooltipData | null>(null);
  ...
  const setTooltipData = useCallback(
    (next: React.SetStateAction<TooltipData | null>) => {
      setRawTooltipData((prev) => {
        const resolved = typeof next === "function" ? next(prev) : next;
        onTooltipChangeRef.current?.(resolved);
        return resolved;
      });
    },
    []
