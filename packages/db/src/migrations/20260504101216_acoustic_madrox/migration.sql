...
--> statement-breakpoint
ALTER TABLE "recurring" ADD COLUMN "category_id" uuid;--> statement-breakpoint
ALTER TABLE "transaction" ADD COLUMN "category_id" uuid;--> statement-breakpoint
ALTER TABLE "transaction" ADD COLUMN "excluded" boolean DEFAULT false NOT NULL;--> statement-breakpoint
...
-- ── SRI-311 phase 1 seed: 16 system category groups + 88 system categories per existing user.
-- Source of truth: packages/server-data/src/transactions/categories/seed.ts
-- Idempotent: skips users who already have any category_group row.
...
target_users AS (
	...
	FROM "user" u
	WHERE NOT EXISTS (
		SELECT 1 FROM category_group cg WHERE cg.user_id = u.id
	)
),
inserted_groups AS (
