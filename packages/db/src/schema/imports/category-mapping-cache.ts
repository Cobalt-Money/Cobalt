 ...
 *   linkRename  — pick existing target + override its display name
...
export const categoryMappingCache = pgTable(
  ...
  {
    action: text("action").$type<"link" | "linkRename" | "create" | "skip">().notNull(),
    confirmedAt: timestamp("confirmed_at", { withTimezone: true }).defaultNow().notNull(),
    /** For linkRename: new display name for the target category. */
