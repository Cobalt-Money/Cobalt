...
export const importsListResponseSchema = z
  .object({
    jobs: z.array(
      z.object({
        createdAt: z.string(),
...
export type ConfirmColumnMapping = z.infer<typeof confirmColumnMappingSchema>;
...
export const confirmAccountMappingSchema = z.discriminatedUnion("kind", [
...
export type ConfirmAccountMapping = z.infer<typeof confirmAccountMappingSchema>;
...
export const confirmCategoryMappingSchema = z.object({
  map: z.record(z.string(), categoryChoiceSchema),
});
export type ConfirmCategoryMapping = z.infer<typeof confirmCategoryMappingSchema>;
...
export const stagedRowsResponseSchema = z.object({
  rows: z.array(stagedRowSchema),
  /** True when the column mapping assigned a tags column — table shows Tags only then. */
  tagsMapped: z.boolean(),
});

/** Single staged row — returned by PATCH /staged-rows/{rowId}. */
export const stagedRowResponseSchema = stagedRowSchema.openapi("ImportStagedRowResponse");
