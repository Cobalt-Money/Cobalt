...
2. Update your `apps/server/.env` file with your PostgreSQL connection details and **Better Auth** settings:
   - `BETTER_AUTH_SECRET` (≥32 chars), `BETTER_AUTH_URL` (e.g. `http://localhost:3000`), `CORS_ORIGIN` — one or more comma-separated web origins (e.g. `http://localhost:3001` or `https://your-web.vercel.app,https://preview-abc.vercel.app`). This must include every browser origin that calls the API; otherwise `/api/auth/*` requests fail with a CORS error.
