...
export async function filterTrackedSymbols(symbols: string[]): Promise<string[]> {
  "use step";
  return await filterTracked(symbols);
}

export async function refreshFinancials(symbol: string): Promise<FundamentalsInsert | null> {
  "use step";
  try {
...
export async function upsertFinancials(rows: FundamentalsInsert[]): Promise<void> {
  "use step";
  await upsertFundamentalsRows(rows);
}

export async function fetchStaleAnalysts(excludeSymbols: string[]): Promise<string[]> {
  "use step";
  return await getStaleAnalysts(excludeSymbols, ANALYST_BATCH_SIZE);
}

export async function refreshAnalysts(symbol: string): Promise<Partial<FundamentalsInsert> | null> {
  "use step";
  try {
    ...
    const msg = error instanceof Error ? error.message : String(error);
    if (msg.includes("429")) {
      throw new RetryableError(`FMP rate limited refreshing analysts ${symbol}`, {
        retryAfter: "1m",
      });
    }
    return null;
...
export async function upsertAnalysts(rows: Partial<FundamentalsInsert>[]): Promise<void> {
  "use step";
