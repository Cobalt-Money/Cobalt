...
export const COBALT_SDK_DESCRIPTION = [
  ...
  '    - cobalt.transactions.list({ startDate?, endDate?, categoryGroup? (string | string[] — category group systemKey(s), e.g. "food_and_drink"), accountType?, minAmount?, maxAmount?, searchQuery?, pendingFilter?, limit?, cursor? }) — returns { transactions, nextCursor, hasMore }; pass `nextCursor` back as `cursor` for the next page',
  "    - cobalt.transactions.create(rows[]) — bulk-insert manual transactions onto user-owned MANUAL accounts. Always pass an array (1–500 rows; wrap single insert in `[]`). Returns `{ ids: [...] }` in input order. All-or-nothing. Each row: { accountId, amount, date, name, categoryId?, currency?, id?, location?, merchantName?, notes?, website? }. Plaid-linked accounts reject. Sign: positive `amount` = inflow / income, negative = outflow / expense. `date` = YYYY-MM-DD. `currency` defaults USD. `location` locks the field against future syncs.",
  ...
  "  Calc (deterministic finance helpers — pure functions, no RPC, safe to loop):",
  "    - cobalt.calc.tvm.fv(rate, nper, pmt, pv?, when?) — future value. rate is per-period (annualRate/12 for monthly). Negative `pmt`/`pv` = cash out; positive return = value at end.",
  "    - cobalt.calc.tvm.pv(rate, nper, pmt, fv?, when?) — present value.",
  "    - cobalt.calc.tvm.pmt(rate, nper, pv, fv?, when?) — payment per period (loan/annuity).",
  "    - cobalt.calc.tvm.nper(rate, pmt, pv, fv?, when?) — number of periods to reach `fv`.",
  "    - cobalt.calc.tvm.rate(nper, pmt, pv, fv?, when?, guess?, tol?, maxIter?) — periodic interest rate.",
  "    - cobalt.calc.tvm.ipmt(rate, per, nper, pv, fv?, when?) — interest portion of payment #per.",
  "    - cobalt.calc.tvm.ppmt(rate, per, nper, pv, fv?, when?) — principal portion of payment #per.",
  "    - cobalt.calc.tvm.npv(rate, values[]) — net present value of irregular cashflows at equal intervals.",
  "    - cobalt.calc.tvm.irr(values[], guess?, tol?, maxIter?) — internal rate of return (equal-period cashflows).",
  "    - cobalt.calc.tvm.mirr(values[], financeRate, reinvestRate) — modified IRR with separate finance/reinvest rates.",
  "    `when` ∈ 0|1 (0 = end of period default, 1 = beginning / annuity-due). Sign convention follows Excel/numpy-financial: outflows negative, inflows positive. Functions are synchronous and pure — do NOT `await` them.",
  "User-scoped calls are automatically restricted to the authenticated user — sandboxed code cannot supply or override `userId`. Most APIs are read-only; accounts.create (manual accounts only), transactions.create, transactions.update, and the tags.* mutators are the writeable surface and require the `cobalt:write` OAuth scope. `cobalt.calc.*` runs in-isolate (no RPC) and is fully synchronous.",
].join("\n");
