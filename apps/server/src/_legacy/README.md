...
Quarantined sandbox runtime + agent-bridge. Replaced by TanStack Code Mode (QuickJS WASM) at `src/ai/agents/finance-agent/code-runtime.ts`.
...
- `api/internal/agent-bridge/registry.ts` — allowlisted route table (now mirrored in `ai/agents/finance-agent/bindings.ts`)
