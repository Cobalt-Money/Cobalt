...
const AccountsLayoutContext = createContext<AccountsLayoutContextValue | null>(
  null
);
...
export function AccountsLayoutProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <AccountsLayoutContext.Provider value={value}>
      {children}
    </AccountsLayoutContext.Provider>
  );
}
...
export function useAccountsLayout() {
  ...
  if (!ctx) {
    throw new Error(
      "useAccountsLayout must be used under AccountsLayoutProvider"
    );
  }
  return ctx;
}
