...
export function groupEvents(dayEvents: IEvent[]): IEvent[][] {
  ...
  for (const event of sortedEvents) {
    ...
    for (const group of groups) {
      const lastEventInGroup = group.at(-1);
      if (!lastEventInGroup) {
        continue;
      }
...
export function calculateMonthEventPositions(
  ...
  for (const day of eachDayOfInterval({ end: monthEnd, start: monthStart })) {
...
export const getEventsForWeek = (events: IEvent[], date: Date): IEvent[] => {
  ...
  const startOfWeekDate = weekDates.at(0);
  const endOfWeekDate = weekDates.at(-1);
