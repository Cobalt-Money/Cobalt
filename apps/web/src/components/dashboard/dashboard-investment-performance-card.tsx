    ...
    () =>
      positions
        .map((pos) => {
          ...
          const pct = costBasis > 0 ? ((pos.openPnl ?? 0) / costBasis) * 100 : 0;
          return { ...pos, pct };
        })
        ...
        <CardContent className="flex min-h-0 flex-1 flex-col gap-5 p-0 px-5 pb-4 sm:px-6">
          ...
          <ul className="flex flex-col gap-0">
            {topHoldings.map((row) => {
              ...
              return (
                <li className="flex min-w-0 items-center justify-between gap-3 py-3" key={row.id}>
                  ...
                  >
                    <PrivateAmount length={5}>{formatPct(row.pct)}</PrivateAmount>
                  </p>
                </li>
              );
            })}
