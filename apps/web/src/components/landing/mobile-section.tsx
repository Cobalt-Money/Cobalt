...
import { FadeUp, LANDING_EASE } from "./fade-up";
import {
          ...
          <div className="mt-2 flex gap-3 overflow-hidden px-4">
            {POSITIONS.map((p) => (
              <div className="flex flex-col items-center" key={p.sym}>
                ...
                <div className="text-[9px] text-emerald-600 dark:text-emerald-400">↗{p.pct}</div>
              </div>
            ))}
          </div>
  ...
  return (
    <section className="px-6 py-24 lg:py-32">
      <div className="mx-auto flex max-w-6xl flex-col items-center gap-16 text-center">
        <FadeUp className="order-2" y={24} duration={0.7}>
          <MobilePhoneFrame />
        </FadeUp>
        <FadeUp className="order-1 flex flex-col items-center" delay={0.05}>
          ...
          <div className="mt-8">
            <motion.a
              className="inline-flex items-center gap-2 rounded-xl bg-foreground px-5 py-3 font-semibold text-background"
